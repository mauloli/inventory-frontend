...
export default function InventoryEditModal(props) {
  const { open, setOpen, devices, locations, getData, resultId } = props;
  const [device, setDevice] = useState(resultId.device.id||0);
  const [type, setType] = useState(resultId.device.type.type || '');
  const [brand, setBrand] = useState(resultId.device.brand||  '');
  const [status, setStatus] = useState(resultId.status || 100,);
  const [form, setForm] = useState({
    hostname: resultId.hostname || '',
    mac_address: resultId.mac_address || '',
    ip_address: resultId.ip_address || '',
    status: resultId.status || 0,
    user_modified: resultId.user_modified || 1,
    id_location: resultId.id_location || 0,
    id_device: resultId.id_device || 0,
    id_user: resultId.id_user || 1
  });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axiosApiIntances.patch(`/inventory/${resultId.id}`, form);
      getData();
      setForm({
        hostname: '',
        mac_address: '',
