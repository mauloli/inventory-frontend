...
export default function EditDevice(props) {
  const { open, setOpen, types, patchData, data } = props;
  const [form, setForm] = useState({
    name: data.name || '',
    brand: data.brand || '',
    id_type: data.id_type || 0
  });
          ...
          <div style={{ display: 'flex', height: '90%', width: '100%', margin: '0px 20px' }}>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-around', height: '100%' }}>
              <div className='modalTableLeft'>
                ...
                <input name='name' type="text" placeholder='Masukan Device' value={form.name} onChange={handleChangeForm} />
              </div>
              <div className='modalTableLeft'>
                <span>
                  Brand
                </span>
                <input type="text" placeholder='Masukan brand' name='brand' value={form.brand}  onChange={handleChangeForm} />
              </div>
              <div className='modalTableLeft'>
                <span>
                  Type
